...
var requesthost = "https://www.washingtonexaminer.com";
...
request(requesthost, function (error, response, body) {
  ...
  pagehead += '<title> Washington Examiner scrape: ' + ftoday + '</title>';
  pagehead += '<link rel="stylesheet" href="../../css/astextnet.min.css">';
  pagehead += '</head><body>';
  pagehead += '<h1 class="px-5 py-3 bg-light">Washington Examiner ' + ftoday + '</h1>';
  var pagefoot = '<hr></body></html>';
  var pagename = 'output/washex/washex_' + fntoday + '.html';

  fs.appendFileSync(pagename, pagehead);


  $('div.TimelinePromo-title a.Link').each(function (index) {
