...
var requesthost = "https://www.lifezette.com";
...
request(requesthost, function (error, response, body) {
  ...
  pagehead += '<title> Lifezette scrape: ' + ftoday + '</title>';
  pagehead += '<link rel="stylesheet" href="../../css/astextnet.min.css">';
  pagehead += '</head><body>';
  pagehead += '<h1 class="px-5 py-3 bg-light">Lifezette ' + ftoday + '</h1>';
  var pagefoot = '<hr></body></html>';
  var pagename = 'output/lifezette/lifezette_' + fntoday + '.html';

  fs.appendFileSync(pagename, pagehead);


  $('h2.title a').each(function (index) {
